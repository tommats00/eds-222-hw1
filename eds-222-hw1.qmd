...
The population of the data sets is the entire city of Lahore, Pakistan.

2.  What are the *samples* in this setting? Please be precise.

The sample of the data set is the different monitoring spots. These are located at their specific latitudinal and longitudinal coordinates.
...
Yes, the differences are as expected. The government data was collected in locations where air quality was thought to be cleaner, so we should expect the mean to be lower.
...
ggplot(data = crowdsourced) +
  geom_point(aes(x = latitude, y = longitude, color ="Crowd_sourced")) +
  geom_point(data = govt, aes(x = latitude, 
                              y = longitude,
                              color = "Government",
                              )) +
...
# can use group_by() and then slice_sample()
```
